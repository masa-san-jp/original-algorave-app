...
**方法2: GitHub Actions(Git 連携を使わずコードで管理したい場合)**
[deploy workflow](.github/workflows/cloudflare-pages.yml) を用意済み。既定では Git 連携(方法1)と二重にならないよう **手動実行(workflow_dispatch)専用**にしてあります。Actions 方式に切り替えるには、workflow 内の `push:` トリガーのコメントを外し、GitHub の **Settings → Secrets and variables → Actions** に `CLOUDFLARE_API_TOKEN`(「Cloudflare Pages : Edit」権限)と `CLOUDFLARE_ACCOUNT_ID` を登録、初回のみ Pages プロジェクト(名前 `original-algorave-app`)を作成します。
